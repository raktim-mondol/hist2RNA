"""
Generate dummy single-cell RNA-seq data for testing hist2scRNA model

This script creates:
1. Synthetic histopathology image patches
2. Spatial coordinates for spots/cells
3. Simulated single-cell gene expression with realistic properties:
...
def generate_gene_expression_zinb(n_spots, n_genes, cell_types, n_cell_types,
    ...
    # Each cell type has higher expression of certain genes
...
def generate_histopathology_patches(n_spots, img_size=224, output_dir='./dummy_data/patches'):
    """
    Generate synthetic histopathology image patches

    Args:
        n_spots: number of patches to generate
        img_size: size of each patch (img_size x img_size)
    ...
    for i in range(n_spots):
        ...
        # H&E staining typically shows purple/blue (hematoxylin) and pink (eosin) regions

        # Background (pink-ish)
        background = np.random.randint(200, 240, size=(img_size, img_size, 3), dtype=np.uint8)
        background[:, :, 0] = np.random.randint(220, 245, size=(img_size, img_size))  # R
        background[:, :, 1] = np.random.randint(180, 220, size=(img_size, img_size))  # G
        background[:, :, 2] = np.random.randint(200, 230, size=(img_size, img_size))  # B

        # Add nuclei (purple/blue blobs)
        ...
        for _ in range(n_nuclei):
            ...
            # Purple/blue color for nucleus
            background[mask, 0] = np.random.randint(100, 150)  # R
            background[mask, 1] = np.random.randint(80, 130)   # G
            background[mask, 2] = np.random.randint(150, 200)  # B
...
def save_dummy_data(n_spots=100, n_genes=2000, n_cell_types=10, output_dir='./dummy_data'):
    ...
    # Plot 1: Spatial coordinates colored by cell type
    scatter = axes[0, 0].scatter(coordinates[:, 0], coordinates[:, 1], c=cell_types, cmap='tab10', s=50)
    axes[0, 0].set_title('Spatial Coordinates (colored by cell type)')
    axes[0, 0].set_xlabel('X coordinate')
    axes[0, 0].set_ylabel('Y coordinate')
    plt.colorbar(scatter, ax=axes[0, 0], label='Cell Type')

    # Plot 2: Expression heatmap (first 50 genes, first 50 spots)
    im = axes[0, 1].imshow(expression[:50, :50], aspect='auto', cmap='viridis')
    axes[0, 1].set_title('Gene Expression Heatmap (50x50)')
    axes[0, 1].set_xlabel('Genes')
    axes[0, 1].set_ylabel('Spots')
    plt.colorbar(im, ax=axes[0, 1], label='Expression')

    # Plot 3: Expression distribution
    axes[1, 0].hist(expression.flatten(), bins=50, edgecolor='black', alpha=0.7)
    axes[1, 0].set_title('Gene Expression Distribution')
    ...
    print(f"   Saved to: {output_dir}/data_visualization.png")
...
if __name__ == "__main__":
    # Generate dummy data with different configurations

    # Small dataset for quick testing
    print("Generating SMALL dataset for quick testing...")
    save_dummy_data(n_spots=50, n_genes=500, n_cell_types=5, output_dir='./dummy_data/small')

    print("\n" + "="*60 + "\n")

    # Medium dataset for realistic testing
    print("Generating MEDIUM dataset for realistic testing...")
    save_dummy_data(n_spots=200, n_genes=2000, n_cell_types=10, output_dir='./dummy_data/medium')
